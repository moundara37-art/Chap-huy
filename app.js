...
function editTransaction(id,type){
  ...
  const personId = x.customer_id;
  const p = people.find(x=>x.id===personId);

  $("editTransactionId").value=id;
  $("editTransactionType").value=type;
  $("editTransactionPerson").textContent=p?.name||"";
  ...
  openDialog("editTransactionDialog");
}
...
$("editTransactionForm").onsubmit=async e=>{
  ...
  if(!id || !["debt","payment"].includes(type) || amount<=0){
    toast("ចំនួនលុយមិនត្រឹមត្រូវ",false);
    return;
  }

  const date=$("editTransactionDate").value||today();
  const note=$("editTransactionNote").value.trim()||null;
  let res;

  if(type==="debt"){
    res=await client.from("debts")
      .update({amount,debt_date:date,note})
      .eq("id",id);
  }else{
    const oldPayment=payments.find(x=>x.id===id);
    if(!oldPayment){
      toast("រកមិនឃើញប្រវត្តិនេះទេ",false);
      return;
    }

    const otherPaid=payments
      .filter(x=>x.customer_id===oldPayment.customer_id && x.id!==id)
      .reduce((a,x)=>a+Number(x.amount||0),0);

    const totalDebt=debts
      .filter(x=>x.customer_id===oldPayment.customer_id)
      .reduce((a,x)=>a+Number(x.amount||0),0);

    const maxPayment=Math.max(0,totalDebt-otherPaid);
    if(amount>maxPayment){
      toast(`ចំនួនសងមិនអាចលើស ${money(maxPayment)}`,false);
      return;
    }

    res=await client.from("payments")
      .update({amount,payment_date:date,note})
      .eq("id",id);
  }

  if(res.error){
    toast(res.error.message,false);
    console.error(res.error);
    return;
  }

  closeDialogs();
  toast("បានកែចំនួនលុយរួចរាល់ ✓");
  await loadAll();
};
...
function history(id){
  ...
  $("historyContent").innerHTML=rows.length?rows.map(x=>{
    ...
    const cls=isDebt?"history-debt":"history-pay";
    const note=x.note?` • ${esc(x.note)}`:"";

    return `<div class="history-row">
      <div class="history-main">
        <div class="history-left">
          <div class="history-type ${cls}">${label}</div>
          <div class="history-meta">${esc(x.date||"")}${note}</div>
        </div>
        <div class="history-actions">
          <div class="history-amt ${cls}">${amount}</div>
          <button type="button" class="btn blue" onclick="editTransaction('${x.id}','${x.type}')">✏️ កែ</button>
        </div>
      </div>
    </div>`;
